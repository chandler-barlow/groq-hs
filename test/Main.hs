...
import qualified Groq.Types.ChatCompletion as Groq
...
tests :: TestTree
tests = testGroup "groq-hs tests"
  [ golden
  ]

roundTripJSON :: 
  forall a. (Aeson.FromJSON a, Aeson.ToJSON a) => 
  BS.ByteString -> 
  Assertion
roundTripJSON bs = 
  let 
...
goldenChatRequest = testCase "Chat completion request roundtrips" $ 
  let 
...
goldenChatResponse = testCase "Chat completion request roundtrips" $ 
  let 
...
golden :: TestTree
golden = testGroup "All aeson instances round trip golden copies"
  [ goldenChatRequest
  , goldenChatResponse
  ]
